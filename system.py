...
class System:
    ...
    def __init__(self, attributes, objects, dec):
        ...
        self.core = set(filter(lambda a: not self.is_redundant(attributes, a), attributes))
    ...
    def abstract_class(self, obj, attrs):
        return frozenset([x.name for x in self.objects.values() if x.ind_to(self.objects[obj], attrs)])
    ...
    def is_reduct(self, X):
        return self.abstract_classes(X) == self.abstract_classes(self.attributes) and \
                not any(map(lambda a: self.is_redundant(X, a), X))
    
    def find_reducts(self):
        add_size = 0
        while len(self.core) + add_size <= len(self.attributes):
            attributes = self.attributes
            reducts = set(filter(self.is_reduct, map(lambda c: frozenset(self.core | set(c)), combinations(self.attributes - self.core, add_size))))
            if(len(reducts) > 0):
                return reducts
            add_size+=1
        return []
    ...
    def __indiff_matrix__(self):
        ...
        for x in self.objects.values():
            ...
            for y in self.objects.values():
                row.append(set([key for key in self.attributes if x.data[key] != y.data[key]]))
            matrix.append(row)
        return matrix
    ...
    def __nondeterministic_rules__(self):
        return list(filter(lambda s: len(self.map_to_decisions(s)) != 1,self.abstract_classes(self.attributes)))
    
    def __recalculate__(self):
        self.core = set(filter(lambda a: not self.is_redundant(self.attributes, a), self.attributes))
        self.reducts = self.find_reducts()
        self.indiff_matrix = self.__indiff_matrix__()
        self.nondeterministic_rules = self.__nondeterministic_rules__()
        self.consistent = len(self.nondeterministic_rules) == 0
